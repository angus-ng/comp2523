...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    if(this._remindersHandler.size() === 0) {
      ...
      const selection = Number(this.getUserChoice("number", true));
      this._remindersHandler.toggleCompletion(selection);
  ...
  private handleModifyReminders(): void {
    if(this._remindersHandler.size() === 0) {
      ...
      Logger.logReminders(this._remindersHandler.reminders);
      const selection = Number(this.getUserChoice("number", true));
      const newDesc = this.getUserChoice("new description", false);
      this._remindersHandler.modifyReminder(selection, newDesc);
      const toggleInput = ReminderApp.checkUserToggleChoice();
      if (toggleInput === true){
        this._remindersHandler.toggleCompletion(selection);
      }
      Logger.log('\n  🏁   Reminder Modified')
