...
export default class RemindersHandler {
    ...
    public getReminder(index: number): Reminder {
        if (this._reminders[index-1] === undefined) {
    ...
    public search(keyword: string): Reminder[] {
        const filterTags = this.searchTags(keyword);
    ...
    public groupByTag(): RemindersGroupingByTag {
        ...
        this._reminders.forEach((reminderObj) => {
            if (groupings[reminderObj.tag] === undefined){
                groupings[reminderObj.tag] = [];
            }
            groupings[reminderObj.tag].push(reminderObj);
        })
        /* Pseudocode:
    ...
    private searchTags(keyword: string): Reminder[] {
        return this._reminders.filter((reminderObj) => reminderObj.tag === keyword);
    }
    ...
    private searchDescriptions(keyword: string): Reminder[] {
        return this._reminders.filter((reminderObj) => reminderObj.description.includes(keyword));
    }
}
